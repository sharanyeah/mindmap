import { DEBUG, project, baseSize, colors, placeholder, fontSize, fontFamily, dragTransplant, renameMode, onFilesDrag, renamedNode, draggedElem, mindMapBox } from './indexLegacy'
...
export function checkBounds(mindMap, mindMapBox, canvas) {
    const tempScale = mindMap.view.scale;
    mindMap.view.scale = 1;

    const canvasElem = canvas ? canvas : getCanvas();
    if (!canvasElem || typeof canvasElem.getContext !== 'function') {
        console.error('Invalid canvas element in checkBounds');
...
export function draw(mindMap, canvasElem=getCanvas()) {
    if (!canvasElem || typeof canvasElem.getContext !== 'function') {
        console.error('Invalid canvas element in draw');
        return;
    }
    const ctx = canvasElem.getContext('2d');
    ...
    drawDottedGrid(ctx, canvasElem, mindMap);
...
export function drawRootText(ctx, mindMap, node) {
    ...
    ctx.font = '600 ' + fontSize * mindMap.view.scale + 'px ' + fontFamily;
...
export function drawNodeText(ctx, mindMap, node) {
    ctx.font = '500 ' + fontSize * 0.75 * mindMap.view.scale + 'px ' + fontFamily;
...
function drawRootNode(ctx, mindMap, node) {
    ...
    ctx.shadowBlur = 8 * mindMap.view.scale;
    ctx.shadowOffsetX = 0;
    ctx.shadowOffsetY = 2 * mindMap.view.scale;

    drawRoundedRect(ctx, point.x - node.boundbox.width / 2 * mindMap.view.scale, point.y - node.boundbox.height / 2 * mindMap.view.scale, node.boundbox.width * mindMap.view.scale, node.boundbox.height * mindMap.view.scale, 8 * mindMap.view.scale);
...
function drawEdge(ctx, mindMap, start, end, node) {
    ...
    ctx.lineWidth = baseSize * 0.25 * mindMap.view.scale;
...
function drawSplash(ctx, mindMap) {
    ...
    ctx.textBaseline = 'middle';
    ctx.textAlign = 'center';
    ctx.font = '600 ' + baseSize * mindMap.view.scale + 'px ' + fontFamily;
...
function drawConnector(ctx, mindMap, x, y, node, state = node.state) {
    ...
    ctx.lineWidth = 1.5 * mindMap.view.scale;
    ...
    if (state == 1) {
        ctx.fillStyle = color;
        drawPlus(ctx, x, y, r * 1.2 * mindMap.view.scale, baseSize / 6 * mindMap.view.scale);
    } else if (state == 2) {
        ctx.fillStyle = color;
        drawCircle(ctx, x, y, r / 3 * mindMap.view.scale, false);
    } else if (state == 3) {
        ctx.fillStyle = color;
        drawMinus(ctx, x, y, r * 1.2 * mindMap.view.scale, baseSize / 6 * mindMap.view.scale);
    }
}
...
function drawDottedGrid(ctx, canvasElem, mindMap) {
    const gridSize = 30;
    const dotSize = 1;
    const dotColor = 'rgba(0, 0, 0, 0.08)';

    const scale = mindMap.view.scale;
    const scaledGridSize = gridSize * scale;

    const offsetX = ((mindMap.view.x * scale) % scaledGridSize);
    const offsetY = ((mindMap.view.y * scale) % scaledGridSize);

    ctx.fillStyle = dotColor;

    for (let x = offsetX; x < canvasElem.width; x += scaledGridSize) {
        for (let y = offsetY; y < canvasElem.height; y += scaledGridSize) {
            ctx.beginPath();
