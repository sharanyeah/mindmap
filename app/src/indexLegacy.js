...
const colors = {};
colors['baseText'] = '#212529';
colors['placeHolderText'] = '#adb5bd';
colors['background'] = '#fafbfc';
colors['border'] = '#dee2e6';
colors['borderCanvas'] = '#ced4da';
colors.branches = ['#A78BFA', '#60A5FA', '#34D399', '#FBBF24', '#F87171', '#EC4899', '#8B5CF6', '#10B981'];
colors.darkColorCoef = 0.65;
colors.lightColorCoef = 0.35;
...
export async function init() {
    // Initialize variables that need DOM access
    workSpaceLoader = new Loader($('#work__loading-animation'));
    baseFontSize = parseInt(getComputedStyle($('body')).fontSize.slice(0, -2)); // Font size in px
    baseSize = baseFontSize * 1.25;
    transplantHoldZone = baseSize / 2;
    bufferOfView = baseSize * 2;
...
function initUI() {
    ...
    const icons = {
        new: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M5 12h14"></path></svg>',
        open: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>',
    ...
    menu.addItem('New', function () { menuItemNewHandler(); }, undefined, false, true, false, icons.new);
    menu.addItem('Open', function () { menuItemOpenHandler(); }, undefined, false, true, false, icons.open);
    menu.addItem('Save', function () { menuItemSaveHandler(); }, undefined, false, true, false, icons.save);
    menu.addItem('Close', function () { menuItemCloseHandler(); }, undefined, false, true, false, icons.close);
    ...
    setFontFamily('-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif', '500');
}
...
function menuItemSaveHandler() {
    ...
    shadowCanvas.width = mindMapBox.width + padding * 2;
    shadowCanvas.height = mindMapBox.height + padding * 2;

    const tempView = mindMap.view;
    mindMap.view = { x: -mindMapBox.x + padding, y: -mindMapBox.y + padding, scale: 1 };
...
function showMindFileUsers() {
    // Don't touch the users div - it's managed by React now
